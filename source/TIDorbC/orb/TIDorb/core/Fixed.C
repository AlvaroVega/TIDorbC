  ...
  if (pointPos == NULL) 
    ...
    { /* Fraction part exists */
      ...
      if (pointPos == decimalPart) 
        ...
        {// Si existe el signo negativo se cont� como un digito.
          intValue = (-fraction);
        } 
      else  
...
char* CORBA::Fixed::to_string()const
{
  ...
  if (_scale >= digitos) { 
    ...
    for (int i = _digits; i < _scale; ++i)
      {// Sumamos 1 a digits por que el 0. que ya se ha a�adido tb cuenta como un digit. creo.
        result[indice++] = '0';
      }
  }
  int r = 0;
...
TIDorb::types::BigInt CORBA::Fixed::transform (CORBA::Short nscale) const
{
  ...
  if (fscale == 0)/* no hay nada que transformar son las mismas escalas */
    ...
    {
      /* si la escala a la que pasamos es mayor, hay que a�adir 0's */ 
...
TIDorb::types::BigInt& CORBA::Fixed::adjust()
{
  if ( (_val.getSign() != 0) && (_scale != 0) ) {
    ...
    while (previo == zero) {
      //El objetivo es reducir la representaci�n interna al maximo.
      div = coc;
    ...
    nscale--; // la ultima divisi�n no cuenta.
    _scale -= nscale;
    _val = div;
  }
  return _val;
                        ...
                        const Fixed a2)
{
  ...
  // El valor que Se dividir� los despazaremos tantos digitos como precisi�n requiera el resultado.
